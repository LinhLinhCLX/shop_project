...
//        dd($viewData);
	...
	public function tintuc1($abc, $xyz){
		 DB::table($abc)->where('id',$xyz)->delete();
	}
    ...
    public function getListService(Request $request)
    {
        ...
        $TagService = DB::table('service')->join('service_tags', 'service.id', '=', 'service_tags.service_id')->where('service.slug', $url)->select('service.id', 'service_tags.name', 'service_tags.id')->get();
    ...
    public function getListNewsTags(Request $request, $keywordtags)
    {
        // dd($keywordtags);
        $tkweb = Blogs::where([['cate_id',2],['active',1]])->paginate(9);


        $newsHot = Blogs::select('name','view','slug')->orderBy('view','DESC')->where([['cate_id',2],['active',1]])->limit(12)->get();

        $TagService = Service::where('name', 'like', "%$keywordtags%")->orWhere('description', 'like', "%$keywordtags%")->orwhere('content', 'like', "%$keywordtags%")->orwhere('summary', 'like', "%$keywordtags%")->orWhere('title', 'like', "%$keywordtags%")->get();
        $newsLikeTags = Blogs::where('name', 'like', "%$keywordtags%")->orWhere('summary', 'like', "%$keywordtags%")->orWhere('detail', 'like', "%$keywordtags%")->paginate(5);
        ...
        $viewData = [
            ...
            'newsLikeTags' => $newsLikeTags,
        ];
        // dd($viewData);
        return view('pages.product.tags_keyword',$viewData);
    }
...

    {
        ...
        $newsHots = Blogs::select('name','view','slug')->orderBy('view','DESC')->where('active',1)->limit(9)->get();
...

    {
        ...
        $seo = Blogs::where([['cate_id',1],['active',1]])->paginate(9);
